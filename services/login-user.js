  ...
  this.get = function () {
    if (!isInit) {
      if (initPromise) {
        ...
        initPromise = _axios2.default.get("user://").then(function (res) {
          console.log("加载当前用户数据", res);
          if (res) {
            Object.assign($loginUser, res);
            // 登录成功
            var userHeadPhoto = res.userHeadPhoto && res.userHeadPhoto !== null && res.userHeadPhoto !== "" ? res.userHeadPhoto : "./public/img/comm/head.png";
