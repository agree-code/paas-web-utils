        ...
        case 'string':
            if (matchdata = Number(date)) {
                ...
                if (matchdata = date.match(/(\d{2,4})?[-\s\\]*(\d{1,2})?[-\s\\]*(\d{1,2})?\s*(\d{1,2})?[-\s\\:]*(\d{1,2})?[-\s\\:]*(\d{1,2})?/)) {
                    ...
                    //检查是否年份
                    matchdata[2]
                        ? date.setFullYear(matchdata[1])
                        : date.setHours(matchdata[1]);
                } else {
                    log.error('时间转换错误 [' + date + ']')
                    return date
                }
            }
            break
...
function format(data, layout) {
    ...
    if (typeof layout !== "string") {
        ...
        layout = layout.replace(/mm/i, month > 9
            ? month
            : '0' + month)
        layout = layout.replace(/m/i, month)
        layout = layout.replace(/dd/i, date > 9
            ? date
            : '0' + date)
        layout = layout.replace(/d/i, date)
        layout = layout.replace(/hh/i, hours > 9
            ? hours
            : '0' + hours)
        layout = layout.replace(/h/i, hours)
        layout = layout.replace(/ii/i, minutes > 9
            ? minutes
            : '0' + minutes)
        layout = layout.replace(/i/i, minutes)
        layout = layout.replace(/ss/i, seconds > 9
            ? seconds
            : '0' + seconds)
